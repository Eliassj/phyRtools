...
loadSpikes <- function(path, triggerfile = NA, minfr = 0) {
    ...
    if (.Platform$OS.type == "windows") {
        infopath <- paste0(path, "\\cluster_info.tsv")
        clusterpath <- paste0(path, "\\spike_clusters.npy")
        timepath <- paste0(path, "\\spike_times.npy")
    } else {
        infopath <- paste0(path, "/cluster_info.tsv")
        clusterpath <- paste0(path, "/spike_clusters.npy")
        timepath <- paste0(path, "/spike_times.npy")
    }
    clusterinfo <- data.table::fread(infopath)[group == "good"]
    if (minfr != 0) {
    ...
    spikesdt <- data.table::data.table(
        "cluster" = as.integer(as.vector(np$load(clusterpath))),
    ...
    if (!is.na(triggerfile)) {
        ...
        triggers <- data.table(
            "t" = triggertimes - 1, # - 1 since index starts at 1 while time starts at 0?
